#!/usr/bin/env python3
"""Fragmenter public RUN ALL v4 with report-backed SNDDATA indexing."""
...
import run_all_executor_v3 as v3
from project_preflight_v1 import require_ready_project
from project_sound_v2 import (
...
def build_run_all_actions_v4(
    ...
    *,
    ...
    old = v3.build_run_all_actions_v3(project, python_executable=python_executable, tools_dir=tools_dir)
    reports = sound_reports_root(project)
    replacements = {
        "snddata_v3": v1.RunAction(
            "snddata_v4",
            "Index SNDDATA Music System",
            "internal",
            internal="analyze_project_snddata_v4",
            inputs=(str(paths.workspace / "sound" / "source" / "data" / "snddata.bin"),),
            outputs=(str(reports / "snddata_music_system_v4.json"), str(reports / "snddata_pipeline_summary_v4.json")),
        ),
        "refresh": v1.RunAction(
            "refresh",
            "Refresh Public Libraries",
            "internal",
            internal="refresh_public_libraries_v4",
...
def _run_internal_v4(
    ...
    if action.internal == "refresh_public_libraries_v4":
    ...
    return v3._run_internal_v3(action, project, paths, callback)


def execute_run_all_v4(
    project: FragmenterProjectV1,
    *,
    reuse: bool = True,
    ...
    for action in actions:
        ...
        try:
            ...
            if reuse and v1.action_reusable(executable_action, state):
                ...
                payload = _run_internal_v4(executable_action, project, paths, callback)
