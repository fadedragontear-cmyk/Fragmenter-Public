#!/usr/bin/env python3
"""Fragmenter public RUN ALL v5 with raw PCM BGM/FOOD preview import."""
...
import run_all_executor_v4 as v4
from project_preflight_v1 import require_ready_project
from project_sound_v3 import (
...
def build_run_all_actions_v5(
    project: FragmenterProjectV1,
    *,
    python_executable: str | Path | None = None,
    tools_dir: str | Path | None = None,
) -> list[v1.RunAction]:
    old = v4.build_run_all_actions_v4(project, python_executable=python_executable, tools_dir=tools_dir)
    replacements = {
        "sound_decode": v1.RunAction(
            "sound_decode",
            "Decode Project Sound + Raw PCM Previews",
            "internal",
            internal="decode_project_sound_v3",
            inputs=next(action.inputs for action in old if action.key == "sound_decode"),
            outputs=next(action.outputs for action in old if action.key == "sound_decode"),
        ),
        "refresh": v1.RunAction(
            "refresh",
            "Refresh Public Libraries",
            "internal",
            internal="refresh_public_libraries_v5",
            inputs=next(action.inputs for action in old if action.key == "refresh"),
            outputs=next(action.outputs for action in old if action.key == "refresh"),
        ),
    }
...
def _progress_bridge(callback: Callable[[dict[str, Any]], None] | None, stage: str):
    def emit(payload: dict[str, Any]) -> None:
        ...
        if payload.get("kind") == "snddata_catalog_progress":
            detail = f"SNDDATA resources {current:,}/{total:,}; sequences {int(payload.get('sequences') or 0):,}"
        elif payload.get("kind") == "raw_pcm_import_progress":
            detail = f"Raw PCM preview {current:,}/{total:,}: {detail}"
        v1._event(
            callback,
            stage=stage,
            kind="progress",
...
def _run_internal_v5(
    ...
    if action.internal == "decode_project_sound_v3":
    ...
    if action.internal == "refresh_public_libraries_v5":
    ...
    return v4._run_internal_v4(action, project, paths, callback)


def execute_run_all_v5(
    project: FragmenterProjectV1,
    *,
    reuse: bool = True,
    ...
    actions = build_run_all_actions_v5(project)
    ...
    for action in actions:
        ...
        try:
            ...
            if reuse and v1.action_reusable(executable_action, state):
                ...
                payload = _run_internal_v5(executable_action, project, paths, callback)
