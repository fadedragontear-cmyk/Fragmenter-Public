#!/usr/bin/env python3
"""Alpha-aware software renderer for Fragmenter CCSF textured previews."""
...
def render_textured_scene(
    ...
    *,
    ...
    def raster(row: tuple[dict[str, Any], list[tuple[float, float, float]], float], *, translucent: bool) -> int:
        ...
        for py in range(min_py, max_py + 1, pixel_step):
            ...
            for px in range(min_px, max_px + 1, pixel_step):
                ...
                # Critical correction: invisible texels do not touch the depth buffer.
                if alpha <= 2:
                    continue
    ...
    # Opaque fragments establish the visible depth surface.
    for row in screen_rows:
        opaque_pixel_writes += raster(row, translucent=False)
    # Approximate correct alpha composition by drawing translucent triangles from
    # far to near without allowing transparent pixels to occlude later geometry.
    for row in sorted(screen_rows, key=lambda item: item[2], reverse=True):
