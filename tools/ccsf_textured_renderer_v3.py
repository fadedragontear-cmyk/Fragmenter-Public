#!/usr/bin/env python3
"""Lower-allocation, alpha-classified CCSF software preview renderer."""
...
def set_preview_background(value: Iterable[int]) -> tuple[int, int, int, int]:
    """Set the process-wide preview background used by every active GUI render."""
...
def set_preview_camera_basis(value: Iterable[float] | None) -> tuple[float, ...] | None:
    """Set the active viewport basis used by GUI software renders.

    The renderer copies this tuple at render start. Obsolete jobs are still rejected
    by the GUI generation/camera checks, while animation frames automatically inherit
    the current camera-relative orientation.
    """
...
def _screen_triangle(
    ...
    *,
    ...
    projected = [_project(position, yaw, pitch, camera_basis) for position in triangle["positions"]]
    return [
...
def _alpha_profile(texture: dict[str, Any]) -> tuple[int, int]:
    cached = texture.get("_renderer_alpha_profile_v3")
    ...
    texture["_renderer_alpha_profile_v3"] = list(profile)
    return profile
    ...
    *,
    ...
    for triangle_index, triangle in enumerate(triangles):
        ...
        for position in triangle["positions"]:
            x, y, _depth = _project(position, yaw, pitch, basis)
            min_x = min(min_x, x)
            max_x = max(max_x, x)
            min_y = min(min_y, y)
            max_y = max(max_y, y)
